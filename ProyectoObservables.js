...
function inicialiarBDD() {
    return new Promise((resolve, reject) => {
        fs.readFile('bdd.json', 'utf-8', (error, contenidoArchivo) => {
            if (error) {
                fs.writeFile('bdd.json', '{"equipos":[]}', (error) => {
                    if (error) {
                        reject({
...
function preguntarNombre(respuestaBDD) {
    ...
    (respuesta) => {
        ...
        if (indiciEquipo === -1) {
            ...
            return rxjs.from(inquirer.prompt(preguntaEdicionEquipo)).pipe(map((respuesta) => {
                respuestaBDD.equipo = {
...
function borrarEquipo(respuestaBDD) {
    return rxjs
        .from(inquirer.prompt(preguntaBuscarEquipo))
        .pipe(mergeMap(// RESP ANT OBS
    (respuesta) => {
        ...
        if (indiceEquipo === -1) {
            ...
            return rxjs.from(promesaEliminar(respuestaBDD.bdd.equipos, indiceEquipo)).pipe(map(() => {
                return respuestaBDD;
            }));
        }
    }));
}
function buscarEquipo(respuestaBDD) {
    return rxjs
        .from(inquirer.prompt(preguntaBuscarEquipo))
        ...
        if (indiceEquipo === -1) {
            ...
            return rxjs.from(promesaBuscar(respuestaBDD.bdd.equipos[indiceEquipo])).pipe(map(() => {
                return respuestaBDD;
            }));
        }
    }));
